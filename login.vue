<template>
    <div class="flex flex-row">
        <div class="w-1/3 flex items-center justify-center px-24 py-24">
            <NuxtLink to="/" class="px-4 py-2 font-medium text-4xl text-white capitalize transition-colors duration-200 transform bg-green-400 rounded-md hover:bg-green-400 focus:outline-none focus:ring focus:ring-green-800 focus:ring-opacity-80">
                SignUp Page
            </NuxtLink>
        </div>
...
export default {
    ...
    methods: {
        async loginUser(){
            try {
                await this.$fire.signInWithEmailAndPassword(
                    thsi.email, this.password
                );
